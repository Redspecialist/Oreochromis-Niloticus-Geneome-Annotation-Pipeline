...
sysopen($OUTPUT, "$ARGV[1].filtered.txt", O_WRONLY| O_CREAT);
sysopen(my $OUTPUT2, "$ARGV[1].filtered.threshold_limited.txt", O_WRONLY| O_CREAT);
...
foreach my $module (keys %module_to_loc){
	...
	if($success >= 5 || $exceptions{$loc}){
	    ...
	    my $info = ($exceptions{$loc} ? $exceptions{$loc} : join(",",@vals));
            my $xms = join(",",@{$loc_to_accession{$loc}});
	    print $OUTPUT "$loc\t$module\n";
	    print $OUTPUT2 "$loc\t@{$loc_to_info{$loc}}[0]\t$xms\t$info\t$module\n";
